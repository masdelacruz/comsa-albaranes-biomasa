...
export function useAlbaranes() {
  ...
  const fetchAlbaranes = async () => {
    ...
    const mapped = albs.map(a => {
      ...
      const firmasObj = {}
      ORDEN_FIRMAS.forEach(rol => {
        if (firmasMap[rol]) firmasObj[rol] = firmasMap[rol]
      })
      ...
      const docsObj = {}
      ordenDocs.forEach(nombre => {
        if (docsMap[nombre]) docsObj[nombre] = docsMap[nombre]
      })
      Object.keys(docsMap).forEach(nombre => {
        if (!docsObj[nombre]) docsObj[nombre] = docsMap[nombre]
      })
      ...
      return {
        ...
        fecha:         a.fecha,
        hora:          a.hora,
        numCamiones:   a.num_camiones,
        tipo:          a.tipo,
        astilladora:   a.astilladora,
        transportista: a.transportista,
        instalacion:   a.instalacion,
        especie:       a.especie,
        tipoBiomasa:   a.tipo_biomasa,
        origen:        a.origen,
        permiso:       a.permiso,
        observaciones: a.observaciones,
        estado:        a.estado,
        mapsOrigen:    a.maps_origen,
        mapsDestino:   a.maps_destino,
        firmas:        firmasObj,
        pesada: {
          entrada:       pesada.entrada        || null,
          salida:        pesada.salida         || null,
          humedad:       pesada.humedad        || null,
          ticketAdjunto: pesada.ticket_adjunto || false,
