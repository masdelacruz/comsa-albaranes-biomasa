...
export default function NuevoAlbaran({ addAlbaran }) {
  ...
  const [form, setForm] = useState({
    ...
    astilladora: '', transportista: '', instalacion: '',
  ...
  return (
    <div className="nuevo-page">
      ...
      <div className="nuevo-content">
        ...
        <div className="form-section card">
          <div className="section-label">Logística y origen</div>
          <div className="form-grid">
            <div className="form-field"><label>Fecha de carga</label><input type="date" value={form.fecha} onChange={e => set('fecha', e.target.value)} /></div>
            <div className="form-field"><label>Hora estimada</label><input type="time" value={form.hora} onChange={e => set('hora', e.target.value)} /></div>
            <div className="form-field"><label>Nº aprox. camiones</label><input type="number" min="1" value={form.numCamiones} onChange={e => set('numCamiones', e.target.value)} /></div>
            <div className="form-field"><label>Especie</label><select value={form.especie} onChange={e => set('especie', e.target.value)}>{ESPECIES.map(s => <option key={s}>{s}</option>)}</select></div>
            <div className="form-field full"><label>Origen biomasa (paraje / término municipal)</label><input type="text" placeholder="Ej: Mas de les Guilles, Arbúcies (Selva)" value={form.origen} onChange={e => set('origen', e.target.value)} /></div>
            <div className="form-field"><label>Maps origen</label><input type="url" placeholder="https://maps.google.com/..." value={form.mapsOrigen} onChange={e => set('mapsOrigen', e.target.value)} /></div>
            <div className="form-field"><label>Maps destino</label><input type="url" placeholder="https://maps.google.com/..." value={form.mapsDestino} onChange={e => set('mapsDestino', e.target.value)} /></div>
          </div>
        </div>

        <div className="form-section card">
          <div className="section-label">Actores</div>
          <div className="form-grid">
            ...
            <div className="form-field">
              ...
              <select value={form.tipoBiomasa} onChange={e => set('tipoBiomasa', e.target.value)}>{TIPOS_BIOMASA.map(t => <option key={t}>{t}</option>)}</select>
            </div>
            <div className="form-field"><label>Permiso / referencia</label><input type="text" placeholder="Nº permiso de corta o SURE" value={form.permiso} onChange={e => set('permiso', e.target.value)} /></div>
            <div className="form-field full"><label>Observaciones</label><textarea placeholder="Ej: Piso móvil 90m³. Acceso restringido a partir de las 14h." value={form.observaciones} onChange={e => set('observaciones', e.target.value)} /></div>
          </div>
        </div>
        ...
        <div className="form-actions">
          ...
          <button className="btn btn-primary" onClick={handleGuardar} disabled={!form.astilladora || !form.transportista || !form.instalacion}>
