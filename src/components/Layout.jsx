...
import { LayoutDashboard, PlusCircle, Leaf, Clock, BarChart2, Settings, LogOut, User, X, Mail, Briefcase, Shield } from 'lucide-react'
import './Layout.css'
          ...
          <div style={{background:'#fff',borderRadius:'var(--radius-xl)',width:'100%',maxWidth:380,boxShadow:'0 20px 60px rgba(0,0,0,0.15)',overflow:'hidden'}}>
            ...
            <div style={{background:'var(--gray-900)',padding:'24px 24px 20px',position:'relative'}}>
              <button
                ...
                style={{position:'absolute',top:16,right:16,background:'none',border:'none',cursor:'pointer',color:'var(--gray-400)',display:'flex',alignItems:'center'}}
              >
                <X size={16} />
              </button>
            ...
            <div style={{padding:'20px 24px'}}>
              ...
              {[
                ...
                { icon: <Shield size={14} />,    label: 'Acceso', value: 'Panel interno — todos los módulos' },
              ...
              <div style={{marginTop:20,paddingTop:16,borderTop:'var(--border)'}}>
                {!confirmLogout ? (
                  <button
                    ...
                    style={{width:'100%',padding:'10px',borderRadius:'var(--radius-md)',border:'1px solid var(--red-100)',background:'var(--red-50)',color:'var(--red-700)',fontSize:13,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,transition:'all 0.15s'}}
                  >
                    <LogOut size={14} /> Cerrar sesión
                  </button>
                ) : (
                  <div style={{background:'var(--red-50)',border:'1px solid var(--red-100)',borderRadius:'var(--radius-md)',padding:'14px'}}>
                    <div style={{fontSize:13,color:'var(--red-700)',fontWeight:500,marginBottom:10,textAlign:'center'}}>
                      ¿Seguro que quieres cerrar sesión?
                    </div>
                    <div style={{display:'flex',gap:8}}>
                      <button
                        onClick={() => setConfirmLogout(false)}
                        style={{flex:1,padding:'8px',borderRadius:'var(--radius-sm)',border:'var(--border)',background:'#fff',fontSize:13,cursor:'pointer'}}
                      >
                        Cancelar
                      </button>
                      <button
                        onClick={handleLogout}
                        style={{flex:1,padding:'8px',borderRadius:'var(--radius-sm)',border:'none',background:'var(--red-400)',color:'#fff',fontSize:13,fontWeight:500,cursor:'pointer'}}
                      >
                        Sí, cerrar sesión
                      </button>
                    </div>
